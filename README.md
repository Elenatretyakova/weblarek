...
`set buttonDisabled (value: boolean)` - блокирует/снимает блокировку с кнопки.
...
`card:select` — при клике на карточку создаёт CardPreview с данными товара, устанавливает текст кнопки («Недоступно» / «Удалить из корзины»), показывает модальное окно.
`product:buy` - при клике на кнопку "купить" добавляет товар или удаляет, если уже добавден в корзину
`basket:open` - при клике на иконку корзины открывает корзину
